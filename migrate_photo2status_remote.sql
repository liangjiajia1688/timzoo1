-- ============================================================
-- TimZoo D1 数据库迁移：photoStatus 三态 → 二态（远程无事务版）
--
-- D1 远程模式不支持 BEGIN TRANSACTION / COMMIT
-- 每条语句独立执行，请按顺序依次运行
--
-- 变更内容：
--   1. CHECK 约束从 IN('pending_shoot','photo-done')
--      改为 IN('pending_shoot', 'published')
--   2. 已有数据中 photoStatus='photo-done' 的记录
--      映射为 'published'
--
-- 使用方式:
--   npx wrangler d1 execute timzoo-db --remote --file=./migrate_photo2status_remote.sql
-- ============================================================

-- Step 1: 创建新表（二态约束 + 对齐当前实际字段）
...
-- Step 2: 迁移数据（photo-done → published 映射）
